...
(deftest test-subset
  (testing "See if subset works as intended."
    (is (= [{:outlook "overcast" :temp "hot"  :humidity "normal" :wind "weak"
             :play-tennis "yes"}
            {:outlook "overcast" :temp "mild" :humidity "high"   :wind "strong"
             :play-tennis "yes"}
            {:outlook "overcast" :temp "cool" :humidity "normal" :wind "strong"
...
(deftest test-most-info-gaining-attr []
  (testing "Testing if most-info-gaining-attribute function works."
    (is (= :outlook
           (#'id3.core/most-info-gaining-attr
            [:humidity :outlook :temp :wind] examples :play-tennis)))))
...
(deftest test-classify []
  (testing "Testing if classify classifies correctly."
    (let [dtree (id3 examples :play-tennis "yes" "no")]
      (is
       (= "no"
          (classify
           {:humidity "high" :outlook "rain" :temp "mild" :wind "weak"}))))))
